...
class Tracker:
    ...
    def __init__(self, model_path: str, model_url: str = None, classes: list[int] = None):
        """Download model if needed and create new Tracker.

        Args:
            model_path (str): either path to existing model or path where model downloaded from model_url will be saved.
    ...
    def run_detection(self, video_path: str, save_path: str, track_args: dict = {"verbose": False}):
        ...
        for iter in trange(length, desc="Detecting"):
            ...
            if success:
                results = self.model.track(frame, persist=True, classes=self.classes, **track_args)
        ...
        unique, inv = torch.unique(boxes[:,4], sorted=True, return_inverse=True)
        boxes[:,4] = inv
        print(f"Found total of {unique.shape[0]} unique objects")
        torch.save({"boxes": boxes, "starts": torch.tensor(starts)}, save_path)
        video.release()

    def add_boxes(self, video_path: str, boxes_path: str, save_path: str, plot_args: dict = {}, ids=None):
        """Add selected bounding boxes to video.

        Args:
            video_path (str): path to original video.
            boxes_path (str): path to bboxes tensors (created in run_detection function).
            save_path (str): path to store resulting video.
            plot_args (dict): additional args for Boxes.plot function.
            ids (list[int] | None): list of object's ids to plot (by default all bboxes are drawn).
        """
        video = VideoReader(video_path)
